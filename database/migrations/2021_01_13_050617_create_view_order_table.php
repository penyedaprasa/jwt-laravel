...
class CreateViewOrderTable extends Migration
{
    ...
    public function up()
    {
        ...
        DB::statement("CREATE VIEW v_orders AS SELECT a.id, d.name, a.event_id, b.event_name, b.event_penyelenggara, b.description, b.image_url, b.tgl_mulai, b.jam_mulai, b.tgl_berakhir, b.jam_berakhir, a.ticket_order, a.total_price_ticket, b.ticket_tersedia, b.total_ticket, b.location_address, c.`name` AS `status`, a.created_at, a.updated_at FROM `event_order` a JOIN v_events b ON `a`.event_id = b.id JOIN order_status c ON a.status_id = c.id JOIN v_users d ON a.user_id = d.id");
